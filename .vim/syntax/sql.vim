...
" syn match   sqlError        " $"                " Space at the end of a line. This is an error??
...
" JOIN keywords
syn match sqlKeyword        "\(inner\|left\|right\|outer\|full\|natural\|cross\) join"
syn match sqlKeyword        "\(left\|right\) outer"
syn match sqlKeyword        "\(left\|right\) inner"
...
" Telsasoft functions
syn match   sqlFunction     "\<\(alarms_cascade_query\|alarms_clear_disregards\|renumber_ranked_table\|renumber_modifs\|renumber_paging\)(\@="
syn match   sqlFunction     "\<\(sites_fill_in_aliases\|sites_propagate_aliases\|sites_propagate_aliases_2\|dominate_severity\|pick_site_id\)(\@="
syn match   sqlFunction     "\<\(cached_objs_timestamp\|divide_or_zero\|maybe_int\|maybe_float\|maybe_pow\)(\@="
syn match   sqlFunction     "\<\(maybe_log\|maybe_div\|cpu_peak_hist\|ch_peak_secs\|extract_sector\|extract\)(\@="
syn match   sqlFunction     "\<\(extract_site\|populate_sites_and_sectors\|lookup_mscid\|lookup_site_from_mscid\|lookup_sect_from_mscid\)(\@="
syn match   sqlFunction     "\<\(format_trunk\|greatest_integer\|greatest_float\|least_integer\|above_zero\)(\@="
syn match   sqlFunction     "\<\(blocking\|offered\|required\|dir_constricted\|site_name_to_sect_name\|date_trunc\)(\@="
syn match   sqlFunction     "\<\(site_name_to_sect_name\|gsm_site_name_to_sect_name\|gsm_site_name_to_sect_name\|site_name_to_sect_name\|follow_site_alias\)(\@="
syn match   sqlFunction     "\<\(grade_retain\|cdr_mobile_number\|cdr_mobile_site\|cdr_other_number\|cdr_esn\)(\@="
syn match   sqlFunction     "\<\(gsm_cdr_mobile_number\|gsm_cdr_other_number\|cdr_npanxx\|merge_site_book_node_values\|merge_site_book_node_items\)(\@="
syn match   sqlFunction     "\<\(alarms_cascade_delete\|alarm_newest_and_lock\|alarm_set\|alarm_history\|alarm_remark\)(\@="
syn match   sqlFunction     "\<\(greatest_integer_idx\|treatment_text\|completion_text\|alcatel_cause_code\|nortel_diag_code\)(\@="
...
syn region sqlString        start=+'+  skip=+\\\\\|\\'+  end=+'+ contains=sqlVariable
" syn region sqlString        start=+`+  skip=+\\\\\|\\`+  end=+`+ contains=sqlVariable " Not optimal for telsasoft reports
